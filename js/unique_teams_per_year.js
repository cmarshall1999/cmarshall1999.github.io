d3.csv('data/unique_teams_per_year.csv').then(data => {
    console.log("Loaded data:", data);  // Debugging

    // Process the CSV data
    data.forEach(d => {
        d.year = new Date(d.year);
        d.number_of_unique_teams = +d.number_of_unique_teams;
    });
    ...
    function update(year) {
        ...
        y.domain([0, d3.max(yearData, d => d.number_of_unique_teams)]);
        ...
        g.selectAll("circle")
            ...
            .enter().append("circle")
            .attr("cx", d => x(d.year))
            .attr("cy", d => y(d.number_of_unique_teams))
            .attr("r", 3)
            .attr("fill", "red")
            .on("mouseover", function(event, d) {
                tooltip.transition()
                    .duration(200)
                    .style("opacity", .9);
                tooltip.html(`Year: ${d.year.getFullYear()}<br>Teams: ${d.number_of_unique_teams}`)
